...
const App = struct {
    ...
    pub fn refreshCredentialForModel(self: *App, model: []const u8) !bool {
        if (!std.mem.startsWith(u8, model, "anthropic-max/") and
            !std.mem.startsWith(u8, model, "openai-codex/") and
            !std.mem.startsWith(u8, model, "xai-direct/")) return false;
        return (try self.auth.selectForModel(self.alloc, model)) orelse false;
    }
    ...
    fn snapshotAndQueuePrompt(
        ...
        if (!credentials.sourceSupportsModel(gateway_credential.source, model_copy)) {
