...
fn loadedCatalogStatusText(state: model_cache_runtime.ModelMenuCatalogState) ?[]const u8 {
    ...
    if (state.access_level == .authenticated) {
        ...
        return switch (source) {
            ...
            .anthropic_fx_login, .anthropic_oauth_token => "Anthropic: authenticated with OAuth.",
            .anthropic_api_key => "Anthropic: authenticated with an API key.",
            .codex_fx_login, .codex_login => "Codex: authenticated with a subscription.",
            .xai_oauth_token => "xAI: authenticated with OAuth.",
            .xai_api_key => "xAI: authenticated with an API key.",
        };
    }
    return null;
}
