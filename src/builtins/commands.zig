...
pub const top_level_specs = [_]TopLevelSpec{
    ...
    .{
        .kind = .login,
        .token = "login",
        .usage = "login [vercel|anthropic|codex|openai-codex|grok|xai] [--device-code|--manual-code]",
        .summary = "Sign in to Vercel or a selected provider",
    },
    .{
        .kind = .logout,
        .token = "logout",
        .usage = "logout [vercel|anthropic|codex|openai-codex|grok|xai]",
...
pub const slash_specs = [_]SlashSpec{
    ...
    .{ .kind = .login, .command = "/login", .help_entry = "/login", .completion_description = "choose an account provider", .presentation_category = .account },
