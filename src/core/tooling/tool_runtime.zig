...
fn executeVisionRequest(
    ...
    if (state.runtime.credential_source) |source| switch (source) {
        ...
        .chatgpt_subscription, .grok_subscription, .anthropic_fx_login, .anthropic_oauth_token, .anthropic_api_key, .codex_fx_login, .codex_login, .xai_oauth_token, .xai_api_key => return error.VisionProviderCredentialUnavailable,
    };
