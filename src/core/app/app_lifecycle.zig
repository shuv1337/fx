...
fn loadStartupStateFromOwnedWorkspace(
    ...
    if (credential_mode) |mode| {
        const direct_model = std.mem.startsWith(u8, state.selected_model, "anthropic-max/") or
            std.mem.startsWith(u8, state.selected_model, "openai-codex/") or
            std.mem.startsWith(u8, state.selected_model, "xai-direct/");
        const resolution = if (direct_model)
            try credentials.resolveForModel(
                alloc,
                transport,
                secret_store,
                mode,
