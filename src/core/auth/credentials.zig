...
/// Resolves the credential for a provider-qualified model route. Unqualified
/// model IDs continue to use the Vercel AI Gateway precedence chain.
...
fn loadPreferredSource(
    ...
    return switch (source) {
        ...
        .anthropic_fx_login, .codex_fx_login, .xai_oauth_token => loadManagedProviderCredential(alloc, transport, provider_oauth.Provider.fromCredentialSource(source).?, mode),
...
pub fn sourceRefreshable(source: Source) bool {
    return source == .fx_login or source == .chatgpt_subscription or source == .grok_subscription or provider_oauth.Provider.fromCredentialSource(source) != null;
}
...
test "provider-qualified models resolve only their direct provider credential" {
    const alloc = std.testing.allocator;
    const env = try CredentialTestEnv.install(alloc, &.{
